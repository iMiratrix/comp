...
try {
    ...
    if (!empty($name_section) && (!empty($name_book) && (!empty($price) && (!empty($img) && (!empty($fio_authors) && (!empty($desck))))))) {
        ...
        $stmt->execute($params);
        $sql = 'INSERT INTO books(name_book,price,desck,fdeck,img) VALUES(:name_book,:price,:desck,:fdeck,:img)';
        $params = ['name_book' => $name_book, 'price' => $price, 'desck' => $desck, 'fdeck' => $fdeck, 'img' => $img];
        $stmt = $pdo->prepare($sql);
        $stmt->execute($params);
